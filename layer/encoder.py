...
class BiFusionLayer(torch.nn.Module):
    def __init__(self, protein_num, drug_num, disease_num, \
        ...
        self.disease_protein = BipartiteGAT(disease_feature_num, protein_feature_num, hidden_dim, heads=1, dropout=0.0,
                                            flow='source_to_target', attention_concat=False, multi_head_concat=False)

        self.ppi_conv = GATConv(2 * hidden_dim, hidden_dim, heads=1, dropout=0.1, concat=False)
    ...
    def forward(self, ppi, drug_protein, disease_protein, drug_feature, disease_feature, protein_feature):
        ...
        protein_feature = self.act(self.ppi_conv(protein_feature, ppi.edge_index))
